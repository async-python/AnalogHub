...
from models.es_models.es_analog import DataAnalog
from models.es_models.es_tool import DataTool
from services.nosql_service import NosqlService, get_nosql_service
from utils.concurrent import run_in_executor
from utils.enums import Filter, Maker, Table
from utils.file_utils import (get_xlsx_path, save_file, verify_required_fields,
                              verify_xlsx_type, verify_zip_type, xlsx_headers)
from utils.queries import page_num_params, page_size_params
from utils.transliterate import prepare_text
...
async def search_list_analogs(background_tasks: BackgroundTasks,
                              ...
                              analog_service: NosqlService = Depends(
                                  get_nosql_service)):
...
@router.post('/search_analog',
             ...
             response_model=list[DataAnalog],
...
async def search_analogs(query: str,
                         ...
                         analog_service: NosqlService = Depends(
                             get_nosql_service)) -> list[DataAnalog]:
    ...
    if search_type == Filter.ngram_search:
        ...
        result = await analog_service.search_analogs_string(
            query_text, page_number, page_size)
    return [DataAnalog(**model.dict()) for model in result]
...
async def search_products(query: str,
                          ...
                          page_size: int = Query(**page_size_params),
                          analog_service: NosqlService = Depends(
                              get_nosql_service)) -> list[DataTool]:
    query_text = prepare_text(query)
    if search_type == Filter.ngram_search:
        ...
        result = await analog_service.search_products_wildcard(
            query_text, maker, page_number, page_size)
    return [DataTool(**model.dict()) for model in result]
